...
GameController::GameController(InfoArea& infoArea, PlayerController& playerController, CharacterVisualizer& playerVisualizer,
                                 CharacterVisualizer& enemyVisualizer, TextRenderer& playerStatusText,
                                 TextRenderer& enemyStatusText) : GameController(
	default_identifier(this), infoArea, playerController, playerVisualizer, enemyVisualizer, playerStatusText, enemyStatusText) {
}
...
GameController::GameController(const std::string& identifier, InfoArea& infoArea, PlayerController& playerController,
	...
	ScriptableBehaviour(identifier), infoArea(infoArea), playerController(playerController), playerVisualizer(playerVisualizer),
...
void GameController::playerAttack() {
	...
	if(evaded) {
		infoArea.AddInfoMessage(string_format("%s (you) tried to attack %s, but missed.", player->getName().c_str(), currentEnemy->getName().c_str()));
	} else {
		infoArea.AddInfoMessage(string_format("%s (you) successfully attacked %s taking away %i health points.", player->getName().c_str(), currentEnemy->getName().c_str(), attackPower));
	}
	currentOrder = !currentOrder;
	waitingForPlayer = false;
...
void GameController::playerHeal() {
	...
	infoArea.AddInfoMessage(string_format("%s (you) recovered %i health points.", player->getName().c_str(), healPower));
	currentOrder = !currentOrder;
	waitingForPlayer = false;
...
void GameController::playerFocus() {
	...
	if (successChance < Difficulty::GetDifficulty().PlayerFocusChance) {
		infoArea.AddInfoMessage(string_format("%s (you) successfully focused. Their next attack is less likely to miss and more likely to cause more damage.", player->getName().c_str()));
...
void GameController::enemyAct() {
	...
	if (enemyChance < Difficulty::GetDifficulty().EnemyAttackChance) {
		...
		if(evaded) {
			infoArea.AddInfoMessage(string_format("%s tried to attack %s (you), but missed.", currentEnemy->getName().c_str(), player->getName().c_str()));
		} else {
			infoArea.AddInfoMessage(string_format("%s successfully attacked %s (you) taking away %i health points.", currentEnemy->getName().c_str(), player->getName().c_str(), attackPower));
		}
		enemyStatusText.setText("Attacking!");
		enemyStatusTextTimer = statusTextTime;
	} else if (enemyChance < Difficulty::GetDifficulty().EnemyHealChance) {
		int healPower = currentEnemy->getHealPower();
		currentEnemy->heal(healPower);
		enemyStatusText.setText("Healing!");
		infoArea.AddInfoMessage(string_format("%s recovered %i health points.", currentEnemy->getName().c_str(), healPower));
		enemyStatusTextTimer = statusTextTime;
	} else if (enemyChance < Difficulty::GetDifficulty().EnemyDoNothingChance) {
		// Do nothing
...
void GameController::nextEnemy() {
	currentEnemy->randomize(Difficulty::GetDifficulty().EnemyBaseAttrPoints);
