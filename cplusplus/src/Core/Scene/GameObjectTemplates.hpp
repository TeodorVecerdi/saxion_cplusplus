...
inline GameObject& MakeButton(const std::string& spritePath, const glm::vec2 size, const std::string& fontName,
                              const std::string& labelText, uint32_t fontSize, TextAlignment textAlignment, sf::Color fillColor) {
	auto* object = new GameObject();
...
inline GameObject& MakeLabel(const std::string& fontName, const std::string& labelText, uint32_t fontSize,
                             TextAlignment textAlignment, sf::Color fillColor) {
	auto* object = new GameObject();
	object->addScriptableBehaviour<TextRenderer>(fontName, labelText, fontSize, textAlignment, fillColor);
	return *object;
}
